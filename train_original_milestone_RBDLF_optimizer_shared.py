...
from config_original_milestone_RBDLF_optimizer_shared import opt
...
def train(**kwargs):
    ...
    # for k,v in s_net.named_parameters():
    #     print(k)
    ...
    for key, value in s_net.named_parameters():
        if not value.requires_grad:
            continue
        if key=='backbone.0.weight' or key=='backbone.1.weight' or key=='backbone.1.bias':
            continue
        if 'classifier1' in key or 'classifier2' in key or 'classifier3' in key:
            print('Yes.')
            params += [{"params": [value], "lr": opt.lr * 10, "weight_decay": opt.weight_decay}]
        else:
            params += [{"params": [value], "lr": opt.lr, "weight_decay": opt.weight_decay}]
